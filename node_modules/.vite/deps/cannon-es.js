...
var Vec3 = class _Vec3 {
   ...
   * @return {Vec3}
   */
  addScaledVector(scalar, vector, target = new _Vec3()) {
...
var Quaternion = class _Quaternion {
   ...
   */
  mult(quat, target = new _Quaternion()) {
    const ax = this.x;
    const ay = this.y;
    const az = this.z;
    const aw = this.w;
   ...
   */
  inverse(target = new _Quaternion()) {
    const x = this.x;
    const y = this.y;
    const z = this.z;
    const w = this.w;
   ...
   */
  conjugate(target = new _Quaternion()) {
    target.x = -this.x;
    target.y = -this.y;
    target.z = -this.z;
    target.w = this.w;
   ...
   */
  slerp(toQuat, t, target = new _Quaternion()) {
    const ax = this.x;
    const ay = this.y;
    const az = this.z;
    const aw = this.w;
   ...
   */
  integrate(angularVelocity, dt, angularFactor, target = new _Quaternion()) {
...
var RigidVehicle = class {
  ...
  setSteeringValue(value, wheelIndex) {
    ...
    const x = axis.x;
    const y = axis.y;
    this.constraints[wheelIndex].axisA.set(c2 * x - s * y, s * x + c2 * y, 0);
  }
  /**
  ...
  step(dt, timeSinceLastCalled, maxSubSteps = 10) {
    if (timeSinceLastCalled === void 0) {
      ...
      while (this.accumulator >= dt && substeps < maxSubSteps) {
        ...
        this.accumulator -= dt;
        substeps++;
        if (performance.now() - t0 > dt * 2 * 1e3) {
          break;
        }
      }
